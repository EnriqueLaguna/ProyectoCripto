...
def cifrarArchivo(informacionArchivo: bytes):
    ...
    texto_encriptado = chachaCipher.encrypt(informacionArchivo.encode())

    nonce = b64encode(chachaCipher.nonce).decode('utf-8')
    ct = b64encode(texto_encriptado).decode('utf-8')
    ...
    return texto_encriptado, result, chacha20_key
...
def decifrarArchivo(infoArchivo:str, key:bytes):
    try:
        ...
        textoDecifrado = cipher.decrypt(cipherText)
